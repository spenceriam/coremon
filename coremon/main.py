            ...
            for key in self.settings:
                if key in config["CoreMon"]:
                    if key in [
                        ...
                        self.settings[key] = config["CoreMon"].getint(
                            key, self.settings[key]
                        )
                    elif key in [
                        "start_minimized",
                        "autostart",
                        "show_individual_cores",
                        "smooth_graphs",
                        "dashboard_avg_only",
                    ]:
                        ...
                        self.settings[key] = config["CoreMon"].get(
                            key, self.settings[key]
                        )
...
"""
    ...
    def update_indicator(self):
        ...
        if temps and loads:
            ...
            if self.settings.get("dashboard_avg_only", False):
                ...
                filtered_temps = (
                    {core_num: temps[core_num]} if core_num in temps else {}
                )
                filtered_loads = (
                    {core_num: loads[core_num]} if core_num in loads else {}
                )
            ...
            if filtered_temps and filtered_loads:
                ...
                load_color = self.get_indicator_color(
                    avg_load, self.settings["load_threshold"]
                )

                # Update indicator label with simple text (no color coding in system tray)
                self.indicator.set_label(
                    f"{avg_temp:.0f}{temp_unit} {avg_load:.0f}%", ""
                )

        # Schedule next update
        GLib.timeout_add_seconds(
            self.settings["update_interval"], self.update_indicator
        )
    ...
    def get_cpu_temps(self):
        """Get CPU temperatures for all cores with universal sensor support"""
        temps = {}
        ...
        try:
            ...
            if hasattr(psutil, "sensors_temperatures"):
                ...
                # Define sensor priority: Intel -> AMD -> ARM -> Generic
                sensor_priority = [
                    ("coretemp", "Core", "Intel"),  # Intel CPUs
                    ("k10temp", ["Tctl", "Tdie", "Tccd"], "AMD"),  # AMD CPUs
                    ("zenpower", ["Tctl", "Tdie", "Tccd"], "AMD"),  # AMD Zen CPUs
                    ("cpu_thermal", "cpu-thermal", "ARM"),  # ARM/Raspberry Pi
                ]

                for sensor_name, label_patterns, sensor_type in sensor_priority:
                    if sensor_name in temps_info:
                        detected_sensor = sensor_type
                        print(
                            f"Detected {sensor_type} temperature sensor: {sensor_name}"
                        )

                        # Handle different label patterns for different sensor types
                        if isinstance(label_patterns, str):
                            # Single pattern (Intel, ARM)
                            for entry in temps_info[sensor_name]:
                                if label_patterns in entry.label:
                                    # Extract core number for Intel
                                    if sensor_type == "Intel":
                                        try:
                                        ...
                                        # For ARM, use sensor index as core number
                                        temps[len(temps)] = entry.current
                        else:
                            # Multiple patterns (AMD)
                            for entry in temps_info[sensor_name]:
                                if any(
                                    pattern in entry.label for pattern in label_patterns
                                ):
                                    # Use index as core number for AMD
                                    temps[len(temps)] = entry.current

                        break  # Stop after finding the first supported sensor

                # If no specific sensor found, try generic fallback
                if not temps and temps_info:
                    print("No specific CPU sensor detected, trying generic fallback...")
                    for sensor_name, entries in temps_info.items():
                        if entries and entries[0].current > 0:  # Valid temperature
                            detected_sensor = "Generic"
                            print(f"Using generic sensor: {sensor_name}")
                            temps[0] = entries[0].current  # Use first available temp
                            break

                if not temps:
                    print("Warning: No temperature sensors detected")
                    detected_sensor = "None"

        except Exception as e:
            print(f"Error getting CPU temps: {e}")
            detected_sensor = "Error"

        # Only use dummy fallback if we really couldn't get any temperatures
        if not temps:
            print("Using dummy temperature values - please check sensor support")
...
class CoreMonWindow(Gtk.ApplicationWindow):
    ...
    def setup_ui(self):
        ...
        self.show_individual_cores_switch.set_active(
            self.app.settings["show_individual_cores"]
        )
        self.show_individual_cores_switch.connect(
            "notify::active", self.on_setting_changed
        )
        ...
        individual_cores_box.pack_end(
            self.show_individual_cores_switch, False, False, 0
        )
        ...
        self.dashboard_avg_only_switch.set_active(
            self.app.settings["dashboard_avg_only"]
        )
        self.dashboard_avg_only_switch.connect(
            "notify::active", self.on_setting_changed
        )
        ...
        system_uptime_label.override_font(
    ...
    def refresh_uptime_only(self):
        ...
        try:
            ...
            self.system_uptime_label.set_text(
                f"System: {self.format_uptime(system_uptime)}"
            )
            self.coremon_uptime_label.set_text(
                f"CoreMon: {self.format_uptime(coremon_uptime)}"
            )
    ...
    def get_system_info(self):
        ...
        try:
            ...
            result = subprocess.run(
                ["neofetch", "--stdout"], capture_output=True, text=True, timeout=10
            )
        ...
        info_lines.append(
            f"Running: {sum(1 for p in psutil.process_iter(['status']) if p.info['status'] == psutil.STATUS_RUNNING)}"
        )
        info_lines.append(
            f"Sleeping: {sum(1 for p in psutil.process_iter(['status']) if p.info['status'] == psutil.STATUS_SLEEPING)}"
        )
    ...
    def update_ui(self):
        ...
        if temps and loads:
            ...
            if filtered_loads:
                ...
                # Apply color coding using CSS classes
                load_color_class = self.get_load_color_class(
                    avg_load, self.app.settings["load_threshold"]
                )
        ...
        # Schedule next update
        GLib.timeout_add_seconds(self.app.settings["update_interval"], self.update_ui)
    ...
    def update_graphs(self):
        ...
        if cores_to_show or -1 in cores_to_show:
            legend = self.temp_ax.legend(
                loc="upper center", bbox_to_anchor=(0.5, -0.25), ncol=4, framealpha=0.9
            )
        ...
        if len(time_labels) > 0:
            ...
            self.load_ax.set_xticklabels(
                [time_labels[i] for i in range(0, len(time_labels), step)]
            )

        # Anchor legend outside plot area (bottom)
        if cores_to_show or -1 in cores_to_show:
            legend = self.load_ax.legend(
                loc="upper center", bbox_to_anchor=(0.5, -0.25), ncol=4, framealpha=0.9
            )
            legend.get_frame().set_facecolor("#2b2b2b")
            for text in legend.get_texts():
